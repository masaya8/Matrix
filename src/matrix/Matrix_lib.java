...
public class Matrix_lib {
	...
	public double [][] getProduct(double a[][], double b[][]){
		...
		for(int i = 0; i < a.length; i++) {
			for(int j = 0; j < b[0].length; j++) {
				answer[i][j] = this.getlnnerProduct(a[i],t[j]);
			}
		}
		return answer;
	}

	public double getlnnerProduct(double a[],double b[]) {
		double answer = 0;
		for(int i = 0; i < a.length; i++) {
			answer += a[i]*b[i];
		}
	...
	public double [] getSolution(double[][] d, double[] e) {
		double answer [][] = new double [1][e.length];
		double tmp [][] = new double [e.length][1];
